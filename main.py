...
STYLE_PROMPTS = {
    "clean": (
        ...
        "без лишнего форматирования и воды."
    ),
    "minimal": (
        "Отвечай максимально коротко — 1-3 предложения, "
        "без вступлений, без воды, только суть."
    ),
    "terminal": (
    ...
    "meme": (
        "Отвечай неформально, с юмором, мемами и эмодзи, "
        "живым языком, как будто общаешься с корешем."
    ),
    "detailed": (
        "Отвечай подробно и развёрнуто, с пояснениями, примерами "
        "и структурой — не бойся длинных ответов, если тема того "
        "требует."
    ),
}
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        for attempt in range(2):
            ...
            if response.status_code == 429:
                logger.warning("Groq rate limit, attempt %s", attempt + 1)
...
async def ask_groq(user_id: int, text: str):
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        answer = await ask_openrouter_vision(user_id, prompt, image_b64)
...
async def startup():
    ...
    logger.info("Ox Alpha AI started: %s", GROQ_TEXT_MODEL)
