...
# Бесплатная модель (актуальный слаг на OpenRouter):
OPENROUTER_MODEL = "deepseek/deepseek-v4-flash:free"
# Если хочешь платную ревизию 0731 — раскомментируй:
...
async def ask_openrouter(
    ...
    for role, content in history:
        # OpenRouter/OpenAI формат: "assistant" вместо "model"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        answer = await ask_openrouter(user_id, text)
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        answer = await ask_openrouter(user_id, prompt, image_b64=image_b64)
        ...
        await update.message.reply_text(
            ...
            "(учти: не все бесплатные модели OpenRouter понимают картинки)"
        )
...
Если отдельного запроса нет, кратко объясни содержимое файла."""
        ...
        answer = await ask_openrouter(user_id, prompt)
...
async def startup():
    ...
    logger.info("Ox Alpha AI started with model: %s", OPENROUTER_MODEL)
