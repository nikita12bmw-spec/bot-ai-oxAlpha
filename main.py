...
async def openrouter_request(
    ...
    if (
        allow_fallback
        and primary_model != OPENROUTER_FALLBACK_MODEL
    ):
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        for index, current_model in enumerate(models_to_try):
            ...
            try:
                ...
                logger.error(
                    "OpenRouter network error with %s: %s",
                    current_model,
                    exc,
                )
...
Не утверждай, что что-то проверил в интернете, если поиск не был выполнен.

Текущая дата: 22 сентября 2026 года.

Пользователь общается с тобой на русском языке, поэтому по умолчанию отвечай на русском.
...
Не используй устаревшие знания модели вместо найденной актуальной информации.
...
async def ask_text(user_id: int, user_text: str):
    persona, style, memory = get_user_settings(user_id)
        ...
        results.append(
            {
                ...
                "url": str(link).strip(),
                "snippet": html.unescape(str(snippet)).strip(),
            }
        )
...
async def search_freeserp(query):
    ...
    results = extract_freeserp_items(data)
...
async def search_ddg(query):
    ...
    results = extract_ddg_items(response.text)
...
async def web_search(query):
    ...
    try:
        results = await search_freeserp(query)
    except Exception as exc:
        logger.warning(
            "FreeSerp failed: %s",
            exc,
    ...
    return await enrich_search_results(results)
...
Когда используешь найденную информацию, можешь указывать источник по названию или URL.
Не придумывай источники и не добавляй URL, которых нет среди найденных.
"""
...
Сохраняй только полезные факты, предпочтения,
долгосрочные настройки и информацию, которую пользователь
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "Ты менеджер долгосрочной памяти Telegram-бота. "
...
async def persona_command(
    ...
    persona = text.partition(" ")[2].strip()
        ...
        if wants_web_search(text):
            ...
            try:
                ...
                await message.reply_text(
                    ...
                    "данные из интернета 😵\n"
                    "Поэтому не буду притворяться, что проверил их."
                )
...
async def video_handler(
    ...
    if video.file_size and video.file_size > 19_500_000:
        await message.reply_text(
    ...
    caption = (
        message.caption
        or "Проанализируй это видео и опиши, что на нём происходит."
    )
...
async def startup():
    logger.info("Starting Ox Alpha AI")
...
async def shutdown():
    logger.info("Stopping Ox Alpha AI")
