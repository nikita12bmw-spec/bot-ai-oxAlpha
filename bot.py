...
async def handle_message(message: Message):
    ...
    # Запускаем запрос к ИИ в фоне, а сами тем временем следим за временем
    # ожидания: обновляем "печатает..." каждые 4 сек (у Telegram индикатор
    # сам гаснет через ~5 сек) и предупреждаем, если ответ задерживается
    task = asyncio.create_task(ask_ai(message.from_user.id, message.text))
    elapsed = 0
    warned = False
